...
export function BrainRecommendation({
  ...
  return (
    <section className="animate-slide-up rounded-xl border border-stone-200/80 bg-gradient-to-br from-amber-50/60 to-white p-6 shadow-sm" style={{ animationDelay: "0.35s" }}>
      ...
      <div className="mt-4">
        ...
        {memory && typeof memory.content === "string" && (
          <div className="mt-4 rounded-lg border border-stone-200/70 bg-white px-4 py-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-stone-400">
              Remember
            </p>
            <p className="mt-1 text-sm text-stone-700">{memory.content}</p>
          </div>
        )}
      </div>
    </section>
