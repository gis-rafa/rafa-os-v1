      ...
      <div className="mt-4 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {projects.map((project) => {
          ...
          return (
            <div
              key={project.id}
              className="rounded-lg border border-stone-100 bg-stone-50/50 p-4 transition-all duration-150 hover:border-stone-200 hover:bg-white hover:shadow-sm"
            >
              <div className="flex items-center gap-2">
                <span className={`inline-block size-2.5 shrink-0 rounded-full ${dotColor}`} aria-hidden="true" />
                <span className="text-sm font-medium text-stone-800 truncate">
                  {project.name}
                </span>
              </div>
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-stone-200" role="progressbar" aria-valuenow={prog} aria-valuemin={0} aria-valuemax={100} aria-label={project.name}>
                <div
              ...
              <div className="mt-1.5 flex items-center justify-between text-xs">
                <span className="text-stone-400 truncate">{project.currentPhase ?? "Active"}</span>
                <span className="font-semibold tabular-nums text-stone-600">{prog}%</span>
              </div>
            </div>
          );
        })}
