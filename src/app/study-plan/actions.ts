...
export async function updateStudyTaskStatusAction(formData: FormData) {
  try {
    ...
    if (!Number.isFinite(day) || !studyTaskStatuses.includes(status)) return;

    await updateStudyTaskStatus({ day, status, userId: user.id });
    revalidatePath("/study-plan");
    revalidatePath("/dashboard");
    revalidatePath("/chat");
  } catch (error) {
    console.error("updateStudyTaskStatusAction failed:", error);
    throw error;
  }
}
